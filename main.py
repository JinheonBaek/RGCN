...
def valid(valid_triplets, model, test_graph, all_triplets, eval_batch_size):

    entity_embedding = model(test_graph.entity, test_graph.edge_index, test_graph.edge_type, test_graph.edge_norm)
    mrr = calc_mrr(entity_embedding, model.relation_embedding, valid_triplets, all_triplets, hits=[1, 3, 10], eval_bz=eval_batch_size)

    return mrr

def test(test_triplets, model, test_graph, all_triplets, eval_batch_size):

    entity_embedding = model(test_graph.entity, test_graph.edge_index, test_graph.edge_type, test_graph.edge_norm)
    mrr = calc_mrr(entity_embedding, model.relation_embedding, test_triplets, all_triplets, hits=[1, 3, 10], eval_bz=eval_batch_size)
